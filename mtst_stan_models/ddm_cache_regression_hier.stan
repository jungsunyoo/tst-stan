...
parameters {
  // subject-level boundary (log scale)
  real mu_log_a;
  real<lower=0> sigma_log_a;
  vector[J] z_log_a;

  // subject-level starting point (logit scale)
  real mu_w;
  real<lower=0> sigma_w;
  vector[J] z_w;

  // subject-level drift intercept
  real mu_v0;
  real<lower=0> sigma_v0;
  vector[J] z_v0;

  // subject-level t0 intercept (logit scale)
  real mu_eta_t0;
...
transformed parameters {
  ...
  for (j in 1:J) {
    ...
    w[j]  = inv_logit(mu_w + sigma_w * z_w[j]);
...
model {
  ...
  mu_w ~ normal(0, 1.0);           // logit space
  sigma_w ~ normal(0, 1.0);
  ...
  for (t in 1:N) {
    ...
    real a_t = fmax(a0[j] * exp(dot_product(row(X_a, t), b_a)), 1e-6);
